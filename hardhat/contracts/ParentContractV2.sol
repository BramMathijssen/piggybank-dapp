...
error ParentContract__ChildHasAllreadyBeenAddedByAParent();
...
contract ParentContractV2 {
    ...
    /* State Variables */
    /// @notice Contract which inherts from the ERC20 contract which is used to create new tokens.
  ...
  */
    ...
    // TODO :  if child already exists then revert
    ...
    function addChild(string memory _name, address _childAddress, address _tokenPreference, uint256 _baseAmount) public hasMinted(_tokenPreference) {
        if (childToParentMapping[_childAddress] != address(0)) {
            revert ParentContract__ChildHasAllreadyBeenAddedByAParent();
        }
     ...
     * @param baseAmount The child's baseAmount allowance
     * @param claimPeriod The child's claimPeriod: daily,weekly,monhtly
     * @return claimableAmount_ The child's claimable allowance
     */
    function calculateClaimableAmount(uint256 baseAmount, ClaimPeriod claimPeriod) public pure returns (uint256 claimableAmount_) {
        if (claimPeriod == ClaimPeriod.DAILY) {
            uint256 dailyAmount = baseAmount - ((baseAmount / 100) * 10);
            claimableAmount_ = dailyAmount / 7;
        } else if (claimPeriod == ClaimPeriod.WEEKLY) {
            claimableAmount_ = baseAmount;
        } else if (claimPeriod == ClaimPeriod.MONTHLY) {
            uint256 monthlyAmount = claimableAmount_ = baseAmount + ((baseAmount / 100) * 10);
     ...
     *  @param tokenAddress The address of the token which the child would like to receive for allowance
     */
